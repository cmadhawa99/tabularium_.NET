...
public class DatabaseSeeder
{
    ...
    {
        var startingSerial = _context.FileRecords.Any()
        ...
        var fileTypes = new[]
        {
            "සාමාන්‍ය (General)", "රහසිගත (Confidential)", "මූල්‍ය (Financial)", "චක්‍රලේඛ (Circular)",
            "පෞද්ගලික (Personal)"
        };
        ...
        var fakeFiles = fileFaker.Generate(count);
        ...
        foreach (var file in fakeFiles)
        {
            ...
            if (file.IsRemoved)
            {
                ...
                file.ToBeRemovedDate = faker.Date.Past();
